...
def edit(id):
    ...
    #     form.title.data = post.body_html[:lambda x:if len(post.body_html)<50 len(post.body_html) else 50]
...
def followers(username):
    ...
    return render_template('followers.html', user=user, title='关注%s的人' % user.username,
...
def followed_by(username):
    ...
    follows = [{'user':item.followed, 'timestamp':item.timestamp}
               for item in pagination.items]
    return render_template('followers.html', user=user, title='%s关注的人' % user.username,
